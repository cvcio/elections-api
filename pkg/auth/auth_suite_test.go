package auth_test

import (
	"crypto/rsa"
	"testing"

	"github.com/cvcio/elections-api/pkg/auth"
	. "github.com/cvcio/elections-api/pkg/auth"
	jwt "github.com/dgrijalva/jwt-go"
	. "github.com/onsi/ginkgo"
	. "github.com/onsi/gomega"
)

func TestAuth(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "Auth Suite")
}

var _ = Describe("Auth", func() {
	var (
		prvKey       *rsa.PrivateKey
		pubKey       *rsa.PublicKey
		a            *Authenticator
		err          error
		signedClaims auth.Claims
		parsedClaims auth.Claims
		userId       = "123"
		org          = "org"
		tknStr       string
	)

	const ( // The key id we would have generated for the private below key
		privateRSAKeyID = "54bb2165-71e1-41a6-af3e-7da4a0e1e2c1"

		// Output of:
		// openssl genpkey -algorithm RSA -out private.pem -pkeyopt rsa_keygen_bits:2048
		privateRSAKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`

		// Output of:
		// openssl rsa -pubout -in private.pem -out public.pem
		publicRSAKey = `-----BEGIN PUBLIC KEY-----

MwIDAQAB
-----END PUBLIC KEY-----`
	)
	BeforeSuite(func() {
		// Parse the private key used to generate the token.
		prvKey, err = jwt.ParseRSAPrivateKeyFromPEM([]byte(privateRSAKey))
		if err != nil {
			Fail("Error parsing private key")
		}

		// Parse the public key used to validate the token.
		pubKey, err = jwt.ParseRSAPublicKeyFromPEM([]byte(publicRSAKey))
		if err != nil {
			Fail("Error parsing public key")
		}
		a, err = auth.NewAuthenticator(prvKey, privateRSAKeyID, "RS256", auth.NewSingleKeyFunc(privateRSAKeyID, pubKey))
		if err != nil {
			Fail("Error creating autenticator")
		}
	})

	Describe("Create token string", func() {
		It("should be able to create a token string from claims", func() {

			signedClaims = auth.Claims{
				Roles:        []string{auth.RoleAdmin, auth.RoleUser},
				User:         userId,
				Organization: org,
			}
			tknStr, err = a.GenerateToken(signedClaims)
			Expect(err).NotTo(HaveOccurred())
		})
		It("should be able to parse a string token to claims", func() {
			parsedClaims, err = a.ParseClaims(tknStr)
			Expect(err).NotTo(HaveOccurred())
		})
		It("should be able to get roles back from parsed token", func() {
			parsedClaims, err = a.ParseClaims(tknStr)
			Expect(err).NotTo(HaveOccurred())
			Expect(len(parsedClaims.Roles)).Should(BeNumerically("==", len(signedClaims.Roles)))
		})
		It("should be able to get correct roles in order back from parsed token", func() {
			parsedClaims, err = a.ParseClaims(tknStr)
			Expect(err).NotTo(HaveOccurred())
			Expect(parsedClaims.Roles[0]).Should(Equal(signedClaims.Roles[0]))
			Expect(parsedClaims.Roles[1]).Should(Equal(signedClaims.Roles[1]))
		})

	})

})
